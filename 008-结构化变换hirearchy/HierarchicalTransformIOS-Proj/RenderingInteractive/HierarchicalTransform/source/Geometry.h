...
struct Geometry {
    ...
    Geometry(VertexPNX *vtx, unsigned short *idx, int vboLen, int iboLen):vao_(0) {
        this->vboLen = vboLen;
        this->iboLen = iboLen;
        ...
        for(int i=0;i<vboLen;i++){
            VertexPNX vData=vtx[i];
            points.push_back(Cvec3(vData.p[0],vData.p[1],vData.p[2]));
        }
        
        for(int i=0;i<iboLen;i++){
            indice.push_back(idx[i]);
        }
    ...
    void draw(const NormalColorShaderState& curSS) {
        if(!vao_){
        ...
        glBindVertexArray(vao_);
        glDrawElements(GL_TRIANGLES, this->iboLen ,GL_UNSIGNED_SHORT,NULL);
