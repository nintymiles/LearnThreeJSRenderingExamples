...
MultiPassTexture::MultiPassTexture():vao_(0){
    ...
    depthTextureShaderState.reset(new DepthTextureShaderState());
...
void MultiPassTexture::Render()
{
    ...
    glUniformMatrix4fv(depthTextureShaderState->matrix_mvp_, 1, GL_FALSE,
                       glmatrix);
    
    glUniform1f(depthTextureShaderState->cameraFar_, -500.f);
    glUniform1f(depthTextureShaderState->cameraNear_, -0.1f);
...
void MultiPassTexture::ApplyShader(ShaderType shader){
    ...
    if (shader == HorizontalBlurShader){
        ...
        program = depthTextureShaderState->program;
    }
